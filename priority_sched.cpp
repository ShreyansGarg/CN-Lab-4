...
public:
    RouterSwitchFabric() : InputQueues(NUM_PORTS, vector<queue<Packet>>(NUM_PORTS)), outputQueues(NUM_PORTS)
    {
        packetCount = 0;
        packetDropped = 0;
            ...
            for (int i = 0; i < NUM_PORTS; i++)
            {
                ...
                for (int j = 0; j < NUM_PORTS; j++)
                {
                    ...
                    if (!InputQueues[j][i].empty())
                    {
                        ...
                        if (p.priority > temp.first)
                        {
                            send = p;
                            temp = {p.priority, p.arrivalTime};
                        }
                        else if ((p.priority == temp.first) && (p.arrivalTime < temp.second))
                        {
                            send = p;
                            temp = {p.priority, p.arrivalTime};
                        }
                ...
                if (temp.first != -1)
                {
                    s.insert(send.inputPort);
                    // cout<<send.inputPort<<" " <<send.priority<<endl;
                    InputQueues[send.inputPort][i].pop();
